...
void max_depth(Tree*root,int cnt,int* maxi){
    if(root==nullptr){
    ...
    max_depth(root->left,cnt+1,maxi);
    max_depth(root->right,cnt+1,maxi);
    return;
...
int main(){
    ...
    int maxi=INT_MIN;
    int cnt=1;
    max_depth(root,cnt,&maxi);
    cout<<"Maximum Depth : "<<maxi<<endl;
