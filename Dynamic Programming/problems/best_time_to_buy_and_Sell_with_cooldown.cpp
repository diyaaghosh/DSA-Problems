...
int solve_by_recursion(int ind,int buy,vector<int>&prices){
    ...
    if(buy==1){
        ...
        return max((prices[ind]+solve_by_recursion(ind+2,1,prices)),solve_by_recursion(ind+1,0,prices));
    }
...
int solve_by_memoization(int ind,int buy,vector<int>&prices,vector<vector<int>>&dp){
    ...
    if(buy==1){
       ...
       return dp[ind][buy]=max((prices[ind]+solve_by_memoization(ind+2,1,prices,dp)),solve_by_memoization(ind+1,0,prices,dp));
    }
